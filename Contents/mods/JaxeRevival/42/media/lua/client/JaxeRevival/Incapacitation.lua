...
local stabilize = function(player)
  ...
  if stats and CharacterStat then
    pcall(stats.set, stats, CharacterStat.POISON, 0)
    pcall(stats.set, stats, CharacterStat.FOOD_SICKNESS, 0)
    pcall(stats.set, stats, CharacterStat.BOREDOM, 0)
    pcall(stats.set, stats, CharacterStat.UNHAPPINESS, 0)
  end
...
JaxeRevival.Incapacitation.applyMechanics = function(player, value)
  ...
  if value then
    ...
    if isClient() then
      ...
      local lastSent = md and md.JaxeRevival_LastIncapSend or lastClientCommandUpdate
      if not lastSent or currentTime - lastSent > (CLIENT_COMMAND_INTERVAL_MINUTES / 60) then
        JaxeRevival.Sync.sendClient(player, JaxeRevival.Sync.INCAPACITATE, JaxeRevival.Sync.getArgsFromTarget(player))
        if md then md.JaxeRevival_LastIncapSend = currentTime end
    ...
    if modData then modData.JaxeRevival_LastIncapSend = nil end
